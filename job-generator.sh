...
curl -s --cookie $COOKIE_JAR $JENKINS_URL'/job/intersystems_iris_contests/createItem?name='$JOB_NAME_TO_GENERATE -H 'Jenkins-Crumb:'$JENKINS_CRUMB -H "Content-Type:text/xml" -u $JENKINS_USER:$JENKINS_PASSWORD --data-binary @/tmp/job-template.xml
